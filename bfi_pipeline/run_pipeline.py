...
from features.extract import extract_all_records_features, get_feature_dims, _FEATURE_NAMES_CACHE, _P1_KEYS, _P2_KEYS, _P3_KEYS, _GL_KEYS
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--skip-hyperopt', action='store_true', help='Skip Optuna hyperparameter search')
    parser.add_argument('--fast', action='store_true', help='Reduced iterations for quick testing')
...
def stage_hyperopt(records: List[Dict], skip: bool=False, fast: bool=False) -> Dict:
    print('\n' + '=' * 70)
    print('STAGE 5: Hyperparameter optimisation')
    print('=' * 70)
    if skip:
        print('  Skipping (using config defaults).')
        return {'best_params': {'lr': cfg.LEARNING_RATE, 'd_proj': cfg.D_PROJ, 'd_hidden': cfg.D_HIDDEN, 'dropout': cfg.DROPOUT, 'aux_lambda': cfg.AUX_LAMBDA, 'batch_size': cfg.BATCH_SIZE}, 'best_val_auroc': float('nan')}
    n_trials = 10 if fast else cfg.OPTUNA_TRIALS
    return run_hyperopt(records, n_trials=n_trials, timeout=cfg.OPTUNA_TIMEOUT)

def stage_train(records: List[Dict], best_params: Dict) -> List[Dict]:
    print('\n' + '=' * 70)
    print('STAGE 6: Leave-one-site-out cross-validation')
    print('=' * 70)
    model_kwargs = {'d_proj': best_params.get('d_proj', cfg.D_PROJ), 'd_hidden': best_params.get('d_hidden', cfg.D_HIDDEN), 'dropout': best_params.get('dropout', cfg.DROPOUT)}
    trainer_kwargs = {'lr': best_params.get('lr', cfg.LEARNING_RATE), 'batch_size': best_params.get('batch_size', cfg.BATCH_SIZE), 'aux_lambda': best_params.get('aux_lambda', cfg.AUX_LAMBDA), 'max_epochs': cfg.MAX_EPOCHS, 'patience': cfg.PATIENCE, 'grad_clip': cfg.GRAD_CLIP, 'weight_decay': cfg.WEIGHT_DECAY}
    fold_results = run_loso_cv(records, model_kwargs=model_kwargs, trainer_kwargs=trainer_kwargs)
    return (fold_results, model_kwargs, trainer_kwargs)

def stage_evaluate(records: List[Dict], fold_results: List[Dict], model_kwargs: Dict, trainer_kwargs: Dict, fast: bool=False) -> Dict[str, Any]:
    print('\n' + '=' * 70)
    ...
    paper_metrics['hyperparameters'] = {**model_kwargs, **{k: v for k, v in trainer_kwargs.items()}}
    print('\n  7a. Per-site metrics ...')
    per_site: Dict[str, Any] = {}
    for fold in fold_results:
        site = fold['test_site']
        met = bootstrap_metrics(fold['class_probs'], fold['true_labels'], fold['patient_ids'], n_iter=100 if fast else cfg.BOOTSTRAP_ITERS)
        per_site[site] = met
    ...
    print(f"    Pooled AUROC={pooled['auroc']:.4f} [{pooled['auroc_ci'][0]:.3f},{pooled['auroc_ci'][1]:.3f}]")
    print('\n  7c. Per-condition metrics ...')
    pid_to_condition: Dict[str, str] = {}
    for rec in records:
        pid_to_condition[rec['patient_id']] = rec['condition']
    condition_groups: Dict[str, Dict] = defaultdict(lambda: {'class_probs': [], 'true_labels': [], 'patient_ids': []})
    for fold in fold_results:
    ...
    for cond, data in condition_groups.items():
        ...
        met = bootstrap_metrics(probs, labels, pids, n_iter=100 if fast else cfg.BOOTSTRAP_ITERS)
    ...
    print(f"    Lead time: {alarm_agg['mean_lead_time_min']:.1f} ± {alarm_agg['std_lead_time_min']:.1f} min")
    print(f"    FP/h:      {alarm_agg['fp_per_h_mean']:.2f} ± {alarm_agg['fp_per_h_std']:.2f}")
    paper_metrics['calibration'] = {'ece': pooled.get('ece', float('nan')), 'brier': pooled.get('brier', float('nan'))}
    print('\n  7f. Feature discrimination (H1) ...')
    families = {'spectral': _P1_KEYS[:5], 'complexity': ['spectral_entropy', 'perm_entropy', 'sample_entropy'], 'coordination': ['coh_alpha_mean', 'wpli_alpha_mean', 'pac_theta_gamma_mi'], 'instability': ['rolling_variance', 'overall_instability'], 'network': ['global_efficiency', 'modularity']}
    ...
    for fam in families:
        ...
        if n < 5:
            feat_disc[fam] = {'median_diff': float('nan'), 'effect_size': float('nan'), 'p_corrected': 1.0}
            continue
        ...
        feat_disc[fam] = result.get('f', {'median_diff': float('nan'), 'effect_size': float('nan'), 'p_corrected': 1.0})
    ...
    sham = run_sham_analysis(records, model_kwargs, trainer_kwargs, n_permutations=3 if fast else cfg.SHAM_REPEATS, max_folds=1 if fast else 2)
    paper_metrics['sham'] = sham
    print(f"    Sham AUROC={sham['auroc_mean']:.4f} ± {sham['auroc_std']:.4f}")
    print('\n  7i. Ablation study ...')
    ablation = run_ablation(records, baseline_metrics=pooled, model_kwargs=model_kwargs, trainer_kwargs=trainer_kwargs, max_folds=1 if fast else 2)
...
def main() -> None:
    ...
    records = stage_load()
    if not records:
        print('\nNo records loaded. Please configure DATA_ROOTS in config.py and ensure at least one dataset is accessible.')
        _save_placeholder_results()
        return
    records = stage_harmonize(records)
    records = stage_preprocess(records)
    records = [r for r in records if len(r.get('seq_labels', [])) > 0 or r.get('features') is not None]
    ...
    if not fold_results:
        ...
        _save_placeholder_results()
        return
    paper_metrics = stage_evaluate(records, fold_results, model_kwargs, trainer_kwargs, fast=args.fast)
...
def _print_summary(metrics: Dict[str, Any]) -> None:
    ...
    print(f"│  Sham AUROC:        {sham.get('auroc_mean', float('nan')):.4f} ± {sham.get('auroc_std', float('nan')):.4f}                     │")
...
def _save_placeholder_results() -> None:
    placeholder = {'status': 'no_data', 'message': 'No valid records were loaded. Configure DATA_ROOTS in config.py.', 'per_condition': {}, 'pooled': {}, 'per_site': {}, 'feature_discrimination': {}, 'sham': {}, 'calibration': {}, 'ablation': {}, 'sensitivity_first_event': {}, 'alarm_logic': {}, 'hyperparameters': {}, 'feature_dim': get_feature_dims()['D_total']}
    out_path = cfg.RESULTS_DIR / 'paper_metrics.json'
    with open(out_path, 'w') as fh:
