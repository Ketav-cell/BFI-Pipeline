"""
train/cross_val.py — Leave-one-site-out cross-validation loop.

For each fold:
  1. Build train / val / test sample lists from pre-extracted features.
  2. Train BFIModel with Trainer.
  3. Predict on the held-out test site.
  4. Collect per-fold metrics.
...
def run_loso_cv(
    ...
    Parameters
    ----------
    records        : all patient records with 'feature_seqs' and 'seq_labels'
    model_kwargs   : overrides for build_model (lr, d_hidden, etc.)
    trainer_kwargs : overrides for Trainer

    Returns
    -------
    List of fold result dicts:
      {fold, test_site, class_probs, bfi_scores, true_labels,
       patient_ids, train_result}
    """
    ...
    print(f"Feature dims: P1={d_p1}  P2={d_p2}  P3={d_p3}  Global={d_global}  "
          f"Total={dims['D_total']}")
    ...
    for fold in folds:
        ...
        print(f"  Samples: train={len(train_samples)}  "
        ...
        fold_results.append({
            "fold":         fold_idx,
            "test_site":    test_site,
            "class_probs":  class_probs,   # (N_test, 3)
            "bfi_scores":   bfi_scores,    # (N_test,)
            "true_labels":  true_labels,   # (N_test,)
            "patient_ids":  patient_ids,   # list[str]
            "train_result": train_result,
            "model":        model,         # kept for ablation / sensitivity
        })
