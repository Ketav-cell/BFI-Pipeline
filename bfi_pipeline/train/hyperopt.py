...
def _objective(trial: 'optuna.Trial', records: List[Dict[str, Any]], fold: Dict[str, Any], dims: Dict[str, int], device: torch.device) -> float:
    ...
    if not train_samples or not val_samples:
        return 0.5
    model = build_model(d_p1=dims['D1'], d_p2=dims['D2'], d_p3=dims['D3'], d_global=dims['D4'], d_proj=d_proj, d_hidden=d_hidden, dropout=dropout)
    trainer = Trainer(model, device, lr=lr, batch_size=batch_size, max_epochs=30, patience=5, aux_lambda=aux_lambda)
    result = trainer.fit(train_samples, val_samples)
    return -result['best_val_auroc']

def run_hyperopt(records: List[Dict[str, Any]], n_trials: int=cfg.OPTUNA_TRIALS, timeout: int=cfg.OPTUNA_TIMEOUT, fold_idx: int=0) -> Dict[str, Any]:
    if not _HAS_OPTUNA:
    ...
    print(f"\nRunning Optuna ({n_trials} trials, timeout={timeout}s) on fold {fold_idx} ({fold['test_site']}) ...")
    study.optimize(lambda trial: _objective(trial, records, fold, dims, device), n_trials=n_trials, timeout=timeout, show_progress_bar=False)
    best = study.best_trial
    best_params = best.params
    best_auroc = -best.value
    print(f'\nBest val AUROC: {best_auroc:.4f}')
    print(f'Best params:    {best_params}')
    return {'best_params': best_params, 'best_val_auroc': best_auroc}