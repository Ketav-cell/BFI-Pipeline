...
class BFIDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        s = self.samples[idx]
        return {'p1': torch.tensor(s['p1'], dtype=torch.float32), 'p2': torch.tensor(s['p2'], dtype=torch.float32), 'p3': torch.tensor(s['p3'], dtype=torch.float32), 'gl': torch.tensor(s['gl'], dtype=torch.float32), 'label': torch.tensor(s['label'], dtype=torch.long)}
...
def records_to_samples(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rec in records:
        ...
        for i, fseq in enumerate(fseqs):
            ...
            samples.append({'p1': fseq['p1'], 'p2': fseq['p2'], 'p3': fseq['p3'], 'gl': fseq['gl'], 'label': label, 'patient_id': pid})
    return samples

class Trainer:

    def __init__(self, model: BFIModel, device: torch.device, lr: float=cfg.LEARNING_RATE, weight_decay: float=cfg.WEIGHT_DECAY, batch_size: int=cfg.BATCH_SIZE, max_epochs: int=cfg.MAX_EPOCHS, patience: int=cfg.PATIENCE, grad_clip: float=cfg.GRAD_CLIP, aux_lambda: float=cfg.AUX_LAMBDA):
        ...
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=max_epochs, eta_min=cfg.LR_MIN)
        self._loss_fn: Optional[BFILoss] = None
    ...
    def _make_loader(self, samples: List[Dict[str, Any]], shuffle: bool) -> DataLoader:
        ds = BFIDataset(samples)
        return DataLoader(ds, batch_size=self.batch_size, shuffle=shuffle, num_workers=0, pin_memory=False)
    ...
    def fit(self, train_samples: List[Dict[str, Any]], val_samples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            print(f'  Epoch {epoch:3d}/{self.max_epochs} | train_loss={train_loss:.4f}  val_loss={val_loss:.4f}  val_AUROC={val_auroc:.4f}  ({elapsed:.1f}s)')
        ...
        return {'best_val_auroc': best_auroc, 'best_epoch': best_epoch, 'train_history': train_history, 'val_history': val_history}

    @torch.no_grad()
    def predict(self, samples: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
        self.model.eval()
        ...
        return (np.concatenate(all_probs, axis=0).astype(np.float32), np.concatenate(all_bfi, axis=0).astype(np.float32))