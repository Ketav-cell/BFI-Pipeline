#!/usr/bin/env python3
"""
quickstart_chbmit.py — Run the BFI feature extraction on CHB-MIT only.
"""
...
def extract_features_simple(window):
    """Extract spectral + complexity + instability features from a window.
    Skip coordination/network features that need specific channel names."""
    feats = {}
    try:
...
def extract_windows_for_patient(record, window_s=10, sfreq=256):
    """Extract pre-event and control feature windows for one patient."""
    eeg = record["eeg"]
    ...
    for evt in events:
        ...
        # Sample every 30 seconds in pre-event window (not every 5s — too slow)
    ...
    # Control: windows far from any seizure
...
def main():
    ...
    # Skip harmonize — CHB-MIT uses bipolar montage, we work with it directly
    ...
    # Only process recordings that have seizure events
    ...
    # Extract pre-seizure features from seizure recordings
    ...
    # Also get control windows from non-seizure recordings
    ...
    output = {
        "dataset": "CHB-MIT",
        "n_recordings": len(preprocessed),
        "n_with_seizures": len(with_events),
        "n_seizure_events": sum(len(r["events"]) for r in with_events),
        "n_pre_event_windows": len(all_pre),
        "n_control_windows": len(all_ctrl),
        "n_features": len(common_keys),
        "feature_discrimination": results,
        "significant_features_count": sig_count,
