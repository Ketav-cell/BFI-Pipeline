...
def load(pdir):
    ...
    if not sf.exists(): return None, None
    sz = {}
    with open(sf) as f:
        for ln in f:
            if "File Name:" in ln: cur = ln.split(":")[1].strip(); sz[cur] = []
            elif "Seizure" in ln and "Start" in ln:
                try: sz[cur].append(int(ln.split(":")[1].split()[0]))
                except: pass
    X, y = [], []
    for edf in pdir.glob("*.edf"):
        if edf.name not in sz: continue
        try:
            ...
            for i in range(0, d.shape[1]-win, win//2):
                ...
                if not np.any(np.isnan(ft)): X.append(ft); y.append(int(lab))
        except: pass
...
for p in sorted(PATH.iterdir()):
    if not p.name.startswith("chb"): continue
    X, y = load(p)
...
for tp in pts:
    ...
    for _ in range(30):
        m.train()
        for X,y in tr: opt.zero_grad(); nn.BCELoss()(m(X.to(DEVICE)).squeeze(), y.to(DEVICE)).backward(); opt.step()
    m.eval()
    pr, lb = [], []
    with torch.no_grad():
        for X,y in te: pr.extend(m(X.to(DEVICE)).squeeze().cpu().tolist()); lb.extend(y.tolist())
    if len(set(lb)) > 1:
