#!/usr/bin/env python3
"""
Burst Suppression Detection from I-CARE EEG
Detects BS patterns (alternating flat/burst) vs continuous EEG
"""
...
def extract_features(signal, fs=FS):
    """Extract features that capture burst suppression characteristics."""
    ...
    # BS-specific features
    
    # 1. Suppression ratio: fraction of signal near zero
    threshold = 0.1 * np.std(signal)
    suppression_ratio = np.mean(np.abs(signal) < threshold)
    
    # 2. Amplitude variance (high in BS due to burst/suppression alternation)
    ...
    if n_windows > 1:
        ...
        amp_cv = np.std(window_amps) / (np.mean(window_amps) + 1e-10)  # Coefficient of variation
    else:
        amp_variance = 0
        amp_cv = 0
    
    # 3. Burst detection: count high-amplitude segments
    burst_threshold = 2 * np.std(signal)
    bursts = np.abs(signal) > burst_threshold
    burst_ratio = np.mean(bursts)
    
    # 4. Zero crossing rate (low during suppression)
    zcr = np.sum(np.diff(np.sign(signal)) != 0) / len(signal)
    
    # 5. Line length (activity measure)
    ...
    return np.array([
        ...
        np.log10(total + 1e-10),
        suppression_ratio, amp_variance, amp_cv,
        burst_ratio, zcr, line_length,
...
def detect_bs_segments(eeg_data, fs=FS, window_sec=10):
    """
    Label segments as BS (1) or normal (0) using heuristics.
    BS characterized by: high suppression ratio + intermittent bursts
    """
    ...
    for start in range(0, n_samples - window_samples, window_samples):
        ...
        # Heuristic BS detection
        threshold = 0.15 * np.std(segment)
        suppression_ratio = np.mean(np.abs(segment) < threshold)
        
        # BS: suppression ratio > 0.3 (30% of signal is flat)
        # Normal: suppression ratio < 0.1
...
def main():
    ...
    results = {
        "dataset": "I-CARE",
        "task": "burst_suppression_detection",
        "n_segments": len(X_bal),
