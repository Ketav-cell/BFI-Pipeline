#!/usr/bin/env python3
"""Patient-specific fine-tuning for higher AUROC"""
import numpy as np
import json
...
def get_patient_features(pdir):
    ...
    if summary.exists():
        ...
        for m in re.finditer(r'File Name:\s*(\S+).*?Seizure.*?Start.*?:\s*(\d+).*?End.*?:\s*(\d+)', txt, re.DOTALL):
...
for tp in patients_with_sz:
    # Split patient data: 30% for fine-tuning, 70% for testing
    ...
    # Build base training set (all other patients)
    ...
    # Fine-tune: add patient-specific data and retrain
    ...
    # Combine base training + patient-specific (weighted more heavily)
...
with open("results/xgb_finetuned.json", "w") as f:
    json.dump({
        "base_auroc": float(np.mean(results_base)),
        "tuned_auroc": float(np.mean(results_tuned)),
        "improvement": float(np.mean(results_tuned) - np.mean(results_base)),
        "std": float(np.std(results_tuned)),
        "n": len(results_tuned)
    }, f)
