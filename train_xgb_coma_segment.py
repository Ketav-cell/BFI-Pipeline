...
def main():
    ...
    results = {'dataset': 'I-CARE', 'task': 'coma_outcome_segment_level', 'n_segments': len(X), 'n_per_class': n_samples, 'auroc_mean': float(np.mean(aurocs)), 'auroc_std': float(np.std(aurocs)), 'accuracy_mean': float(np.mean(accs)), 'accuracy_std': float(np.std(accs))}
