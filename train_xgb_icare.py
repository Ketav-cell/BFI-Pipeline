...
def extract_features(eeg_data, fs=FS, n_channels=10):
    ...
    for ch in range(n_ch):
        ...
        all_features.extend([rel_delta, rel_theta, rel_alpha, rel_beta, theta_alpha, delta_alpha, np.log10(total_power + 1e-10), variance, sig_skew, sig_kurt, line_length, zero_cross, delta, theta, alpha])
...
def main():
    ...
    for train_idx, test_idx in loo.split(X):
        ...
        model = xgb.XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.1, random_state=42, use_label_encoder=False, eval_metric='logloss', verbosity=0)
    ...
    results = {'dataset': 'I-CARE', 'task': 'coma_outcome', 'auroc': float(auroc), 'accuracy': float(acc), 'n_patients': len(y_true), 'n_good': int(np.sum(y_true == 0)), 'n_poor': int(np.sum(y_true == 1))}
