...
def compute_features(data, fs):
    ...
    if total == 0:
        ...
        bp = {f'rel_{b}': trapezoid(psd[(freqs >= lo) & (freqs <= hi)], freqs[(freqs >= lo) & (freqs <= hi)]) / total for b, (lo, hi) in bands.items()}
    ...
    return {**bp, 'spectral_entropy': float(-np.sum(psd_norm * np.log2(psd_norm + 1e-10))), 'spectral_slope': float(slope), 'std': float(np.std(data)), 'line_length': float(np.sum(np.abs(diff1))), 'hjorth_mobility': float(mobility)}
...
def parse_seizures(sf):
    with open(sf) as f:
        content = f.read()
    seizures = []
    for block in re.split('Seizure n\\s*\\d+', content)[1:]:
        fm = re.search('File name:\\s*(\\S+)', block)
        rm = re.search('Registration start time:\\s*(\\d+)[.\\:](\\d+)[.\\:](\\d+)', block)
        sm = re.search('Seizure start time:\\s*(\\d+)[.\\:](\\d+)[.\\:](\\d+)', block)
        if fm and rm and sm:
...
def main():
    ...
    json.dump({'dataset': 'Siena', 'n': len(all_data), 'base_mean': round(np.mean(base_aucs), 3), 'tuned_mean': round(np.mean(tuned_aucs), 3)}, open('results/siena_xgb_results.json', 'w'), indent=2)
