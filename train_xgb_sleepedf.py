...
def extract_features(signal, fs):
    ...
    return np.array([delta / total, theta / total, alpha / total, beta / total, gamma / total, theta / (alpha + 1e-10), delta / (alpha + 1e-10), (alpha + beta) / (delta + theta + 1e-10), np.log10(total + 1e-10), np.log10(delta + 1e-10), np.log10(alpha + 1e-10), np.var(signal), skew(signal), kurtosis(signal), np.mean(np.abs(np.diff(signal))), np.sum(np.diff(np.sign(signal)) != 0) / len(signal), spectral_entropy, activity, mobility, complexity])
...
def main():
    ...
    results = {'dataset': 'Sleep-EDF', 'task': 'wake_vs_sleep_classification', 'n_subjects': loaded, 'n_epochs': len(X), 'n_per_class': n_samples, 'auroc_mean': float(np.mean(aurocs)), 'auroc_std': float(np.std(aurocs)), 'accuracy_mean': float(np.mean(accs)), 'accuracy_std': float(np.std(accs))}
