...
Train on CHB-MIT seizures, test on Sleep-EDF and I-CARE WITHOUT retraining

Novel claim: Seizure-trained features generalize to other neurological conditions
"""
...
# Universal feature extraction (same for all datasets)
def extract_features(signal, fs):
    """Extract 20 universal EEG features."""
    ...
    return np.array([
        ...
        np.mean(np.abs(np.diff(signal))),  # line length
...
def load_chbmit_data(max_patients=10, max_files_per_patient=5):
    """Load CHB-MIT seizure data: pre-ictal (1) vs interictal (0)."""
        ...
        for edf_file in edf_files:
            try:
                ...
                for start_sample in range(0, len(signal) - window_samples, window_samples):
                    ...
                    # Label: 1 if within 60 sec before seizure, 0 otherwise
...
def load_sleepedf_data(max_subjects=30):
    """Load Sleep-EDF: Wake (0) vs Sleep (1)."""
...
# ========== I-CARE BURST SUPPRESSION LOADING ==========
def load_icare_bs_data():
    """Load I-CARE: Normal (0) vs Burst Suppression (1)."""
...
def main():
    ...
    # ===== TEST 2: I-CARE Burst Suppression =====
